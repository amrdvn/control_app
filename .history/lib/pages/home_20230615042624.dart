...
Future<void> uygulamaKullaniminiGonder(String uid) async {
  try {
    ...
    DocumentReference userRef = logsRef.doc(uid);
    CollectionReference kullanimRef = userRef.collection('uygulama_kullanim');

    // 30 gün önce ve bugün arasında bir zaman aralığı belirle
    DateTime endDate = DateTime.now();
    DateTime startDate = endDate.subtract(Duration(days: 30));

    // Kullanım iznini sağla
    await UsageStats.grantUsagePermission();

    // Kullanım istatistiklerini sorgula
    List<UsageInfo> usageStats = await UsageStats.queryUsageStats(startDate, endDate);

    // Kullanım sürelerini Firestore'a gönder
    for (UsageInfo usageInfo in usageStats) {
      if (usageInfo.usage != null && usageInfo.usage.inMinutes > 0) {
        String uygulamaAdi = usageInfo.packageName!;
        int sure = usageInfo.usage!.inMinutes;

        // Firestore'a veri ekle
        await kullanimRef.add({

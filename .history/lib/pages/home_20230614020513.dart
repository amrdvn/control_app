...
class _HomeScreenState extends State<HomeScreen>  {
  ...
  static const platform = MethodChannel('com.example.myapp/disableApp');
...
void disableApp(String packageName) {
  try {
    platform.invokeMethod('disableApp', {'packageName': packageName});
  } on PlatformException catch (e) {
    print("PlatformException: ${e.message}");
  }
}

void disableApp(String packageName) async {
    try {
      final String result = await platform.invokeMethod('disableApp', <String, dynamic>{
        'packageName': packageName,
      });
      print(result);
    } on PlatformException catch (e) {
      print("Failed to disable app: '${e.message}'.");
    }
  }


  
void applyAppRestrictions() async {
  Directory? appDirectory = await getExternalStorageDirectory();
  String appPath = appDirectory!.path;
  String filePath = '$appPath/uygulama_kisit.json';
  ...
  for (var app in data) {
    if (app['kisit'] == 1) {
      ...
      // Uygulama kilitleme mantığını burada uygulayabilirsiniz.
      // Örneğin, başlatılan uygulamanın adını kontrol ederek kilit ekranını gösterebilirsiniz.

      // Kilit ekranını göstermek için flutter_app_lock paketini kullanın
      AppLock.of(context)?.showLockScreen();

      // Kilit ekranı başarılı bir şekilde açıldıktan sonra devam edin
      AppLock.of(context)?.didUnlock();
      DeviceApps.listenToAppsChanges().where((ApplicationEvent event) => event.packageName == appName);
      DeviceApps.openAppSettings(appName);
      print('Uygulama kilitlendi: $appName');
      disableApp(appName);
      print(DeviceApps.listenToAppsChanges().where((ApplicationEvent event) => event.packageName == appName));

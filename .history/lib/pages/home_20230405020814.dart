...
Future<void> sendAppUsageDataToFirestore() async {
  ...
  // FlutterAppUsage instance'ını oluştur
  FlutterAppUsage appUsage = FlutterAppUsage();

  // Bugünden başlayarak son 7 gün boyunca kullanılan uygulama verilerini al
  DateTime endDate = DateTime.now();
  DateTime startDate = endDate.subtract(Duration(days: 6));

  // Uygulama kullanım verilerini al
  Map<String, double> appUsageData = await appUsage.fetchUsage(startDate, endDate);

  // Firestore'a uygulama kullanım verilerini gönder
  await firestore.collection('app_usage').add(appUsageData);
}

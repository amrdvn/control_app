...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void initState() {
    ...
    super.initState();
    // uygulama kısıtlma kısmı
  DeviceApps.onAppOpen.listen((ApplicationWithIcon app) {
    checkRestriction(app.appName);
  });
...
// Uygulama Kilitleme Kısmı //
// Uygulama Kilitleme Kısmı //
void checkRestriction(String appName) async {
  FirebaseAuth auth = FirebaseAuth.instance;
  String? uid = auth.currentUser?.uid;
  CollectionReference logsCollection =
      FirebaseFirestore.instance.collection('logs');
  ...
  try {
    ...
    if (querySnapshot.docs.isNotEmpty) {
      // Retrieve the restriction value
      int restriction = querySnapshot.docs[0].data()['kisit'];

      if (restriction == 1) {
        // Show a lock message using fluttertoast package

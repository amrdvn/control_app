...
Future<void> sendAppUsageStatsToFirestore() async {
  // Firestore instance oluştur
  final FirebaseFirestore _firestore = FirebaseFirestore.instance;

  // Kullanım istatistiklerine erişim iznini ver
  ...
  // Firestore'a kullanım istatistiklerini gönder
  for (EventUsageInfo event in queryEvents) {
    AppUsageInfo appUsageInfo = AppUsageInfo(
      packageName: event.packageName!,
      appName: event.appName!,
      totalTimeInForeground: event.totalTimeInForeground?? 0,
      lastTimeUsed: event.lastTimeUsed,
    );

    await _firestore.collection('app_usage_stats').add(appUsageInfo.toMap());
  }
}

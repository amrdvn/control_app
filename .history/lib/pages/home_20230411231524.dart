...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void _listenToEvents() {
    _usageStatsCollection.snapshots().listen((event) {
      // Etkinlikleri listeye ekle
      events = event.docs
          .map((doc) => EventUsageInfo.fromMap(doc.data()))
          .toList();

      // Liste değiştiğinde yeniden çiz
      setState(() {});
    });
  }

  void _startListeningToEvents() {
    // Timer ile etkinlikleri dinle
    Timer.periodic(_selectedDuration!, (timer) {
      _listenToEvents();
    });
  }

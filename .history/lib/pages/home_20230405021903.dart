...
Future<void> sendUsageStatsToFirestore() async {
  // Firestore bağlantısını başlat
  FirebaseFirestore firestore = FirebaseFirestore.instance;

  // Kullanım istatistiklerini al
  List<AppUsageInfo> usageStats = await UsageStats.queryUsageStats(
    DateTime.now().subtract(Duration(days: 7)), // Son 7 gün için kullanım istatistiklerini alabilirsiniz
    DateTime.now(),
  );

  // Kullanım istatistiklerini Firestore'a gönder
  for (AppUsageInfo usageInfo in usageStats) {
    // Kullanım süresini dakikaya çevir
    int usageDurationMinutes = (usageInfo.usageDuration.inMilliseconds / (1000 * 60)).ceil();

    // Firestore'a kullanım istatistiğini ekleyin
    await firestore.collection('usage_stats').add({
      'app_name': usageInfo.packageName,

...
Future<Position> getCurrentLocation() async {
  ...
  // Konum servislerinin açık olup olmadığını kontrol edin
  serviceEnabled = await Geolocator.isLocationServiceEnabled();
  if (!serviceEnabled) {
    // Konum servisleri kapalıysa, kullanıcıyı konum servislerini açmaya yönlendirin
  ...
  if (permission == LocationPermission.denied) {
    // Konum izni verilmemişse, kullanıcıyı konum iznini vermeye yönlendirin
    permission = await Geolocator.requestPermission();
    if (permission != LocationPermission.whileInUse &&
        permission != LocationPermission.always) {
      // Konum izni verilmediyse, kullanıcıyı konum iznini vermeye yönlendirin
...
Future<void> sendLocationToFirestore(Position position, String uid) async {

...
Future<void> uygulamaKullaniminiGonder(String uid) async {
  // Uygulama kullanım süresini al
  List<AppUsageInfo> appUsageList = await AppUsageTracker.getAppUsage(
    DateTime.now().subtract(Duration(days: 1)), // Son 1 günü kontrol et
    DateTime.now(), // Şu anki zamana kadar olan süreyi kontrol et
  );

  // Firestore referansını al
  final logsCollection = FirebaseFirestore.instance.collection('logs');
  final uygulamaKullanimCollection = logsCollection.doc(uid).collection('uygulama_kullanim');

  // Uygulama kullanım sürelerini Firestore'a gönder
  for (var appUsage in appUsageList) {
    await uygulamaKullanimCollection.doc(appUsage.packageName).set({
      'uygulama_adi': appUsage.appName,
      'sure': appUsage.usage.inMinutes,
    });
  }
}

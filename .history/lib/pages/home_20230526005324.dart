...
import 'package:intl/intl.dart'; // intl package imported
...
class _HomeScreenState extends State<HomeScreen> with LockScreenMixin {
  ...
  Widget build(BuildContext context) {
    return LockScreen(
      ...
      unlockScreen: () {
        // Perform actions when the app is unlocked
        setState(() {});
      },
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: <Widget>[
                  ...
                  Text(
                    "${loggedInUser.ad} ${loggedInUser.soyad}",
                    style: TextStyle(
  ...
  @override
  void LockScreenMixin(BuildContext context) {
    FirebaseFirestore.instance
        .collection('logs')
        .doc(uid)
        .collection('uygulama_kisit')
        .doc('your_app_package_name') // Replace with your app package name
        .get()
        .then((snapshot) {
      if (snapshot.exists && snapshot.data()?['kisit'] == 1) {
        Alert(
          context: context,
          title: 'App is locked',
          desc: 'Please enter the password to unlock.',
          buttons: [
            DialogButton(
              child: Text(
                'Unlock',

...
Future<void> uygulamaKisitKontrol(String appName) async {
  try {
    Directory? appDirectory = await getExternalStorageDirectory();
    String appPath = appDirectory!.path;
    String filePath = '$appPath/uygulama_kisit.json';
    ...
    final List<dynamic> appData = jsonDecode(contents);
...
void runAppWithBackgroundAppInfo() {
  const MethodChannel appInfoChannel = MethodChannel('app_info_channel');
  appInfoChannel.setMethodCallHandler((MethodCall call) async {
    if (call.method == 'onForegroundAppChanged') {
      String foregroundApp = call.arguments as String;
      // Burada alınan arka plan uygulama adını kullanabilirsiniz.
      print('Arka planda çalışan uygulama adı: $foregroundApp');
      uygulamaKisitKontrol(foregroundApp);
    }
  });
}
...
//uygulama kullanım istatistiği
  ...
  void zamanAyarla(Duration period) {
    Timer.periodic(period, (timer) {
      ...
      uygulamaKisitKontrol('Calculator');
    });
  }

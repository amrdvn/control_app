...
import 'package:control_app/model/uygulama_kullanimi.dart' as Model;
...
Future<void> uygulamaKullaniminiGonder(String uid) async {
  ...
  List<Model.AppUsageInfo> infoList;

  try {
    infoList = (await AppUsage().getAppUsage(startDate, endDate)).cast<AppUsageInfo>();
  ...
  for (var info in infoList) {
    await uygulamaKullanimCollection.doc(info.packageName).set({
      'uygulama_adi': info.appName,
      'sure': info.usage.inMinutes,
    });
  }
}

...
import 'package:usage/usage.dart';import 'package:control_app/model/uygulama_kullanimi.dart';
...
Future<void> sendUsageDataToFirestore() async {
  // Kullanım verilerini almak için Usage kütüphanesini kullanın
  Usage usage = Usage();
  await usage.init();

  // Kullanıcının uygulamalarını kullanım süreleri ile birlikte alın
  Map<String, UsageInfo> appUsageMap = await usage.getAllAppsUsage();

  // Firestore instance'ını alın
  FirebaseFirestore firestore = FirebaseFirestore.instance;

  // Firestore'a kullanım verilerini gönder
  CollectionReference usageCollection = firestore.collection('usage');
  appUsageMap.forEach((appName, usageInfo) {
    UsageData usageData = UsageData(
      appName: appName,
      usageDuration: usageInfo.usage,
    );
    usageCollection.add(usageData.toMap());
  });

  // Usage kütüphanesini kapatın
  await usage.close();
}

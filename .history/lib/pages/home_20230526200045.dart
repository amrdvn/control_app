...
void showAlertIfKisitIsOne() {
// Get the value of the kisit field.
final kisit = FirebaseDatabase.instance
.reference()
.child('logs')
.child(uid)
.child('uygulama_kisit')
.get()
.then((snapshot) => snapshot.value['kisit']);

// If the value of the kisit field is 1 and the app is open, then show an alert to the user saying that the app is locked.
if (kisit == 1 && WidgetsBinding.instance.window.hasFocus) {
showDialog(
context: context,
builder: (context) {
return AlertDialog(
title: Text('App is locked'),
content: Text('You are not allowed to open this app.'),
actions: [
TextButton(
child: Text('OK'),
onPressed: () {

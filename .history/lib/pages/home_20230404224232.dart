...
Future<void> getRecentCalls() async {
  ...
  var calls = await CallLog.g;
  var recentCalls = calls.take(5).toList();

  // Son 5 arama kaydını yazdırın
  recentCalls.forEach((call) {
    print('Telefon numarası: ${call.number}');
    print('Konuşma süresi: ${call.duration}');
    print('Tarih: ${call.timestamp}');
  });
}
Future<void> sendCallLogsToFirestore(List<CallRecord> recentCalls) async {
  // Firestore'a bağlanın ve "call_logs" adlı bir koleksiyon oluşturun
  var collection = FirebaseFirestore.instance.collection('call_logs');

  // Son 5 arama kaydını Firestore'a gönderin
  recentCalls.forEach((call) {
    collection.add({

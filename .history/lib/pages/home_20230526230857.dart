...
void showPasswordScreen(BuildContext context) {
  showDialog(
    context: context,
    builder: (context) => AlertDialog(
      title: Text('Uygulama Erişimi'),
      content: Column(
        children: [
          TextFormField(
            obscureText: true,
            ...
            // Şifre girişi alındıktan sonra işlenecek mantığı buraya ekleyebilirsiniz.
            onFieldSubmitted: (password) {
              if (checkPassword(password)) {
                Navigator.pop(context, true); // Şifre doğruysa ekranı kapat ve uygulamaya devam et
              } else {
                // Şifre yanlışsa hata mesajı göster ve uygulamayı kapat
                showDialog(
                  context: context,
                  builder: (context) => AlertDialog(
                    title: Text('Hata'),
...
// Kısıtlı uygulamaların kontrolünü gerçekleştiren fonksiyon
void checkAppRestrictions() async {
  ...
  try {
    ...
    if (querySnapshot.docs.isNotEmpty) {
      // Kısıtlı uygulamalar bulundu, şifre ekranını göster
      bool passwordCorrect = await showDialog(
        context: context,
        builder: (context) => showPasswordScreen(context),
      );

      if (!passwordCorrect) {
        // Şifre yanlış, uygulamayı kapat
        SystemNavigator.pop();
      }
    }
  } catch (e) {

...
class _HomeScreenState extends State<HomeScreen> with WidgetsBindingObserver {
  Accessibility Service _accessibilityService;
  List<AppRestriction> _appRestrictions = [];
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    if (state == AppLifecycleState.resumed) {
      _checkAppRestrictions();
    }
  }

  void _initializeAccessibilityService() async {
    _accessibilityService = AccessibilityService();
    await _accessibilityService.initService();

    _accessibilityService.onAccessibilityEvent.listen((AccessibilityEvent event) {
      if (event.eventType == AccessibilityEventType.windowStateChanged) {
        final String currentApp = event.packageName;
        final int eventType = event.eventTypeRaw;
  ...
  void _checkAppRestrictions() async {
    final String currentApp = await _accessibilityService.getForegroundApp();
    final List<AppRestriction> restrictions = await _loadAppRestrictions();

    for (AppRestriction restriction in restrictions) {
      if (restriction.appName.toLowerCase() == currentApp.toLowerCase()) {
        if (restriction.restriction == 1) {
          // Kisit degeri 1 ise home ekranina don
  ...
  Future<List<AppRestriction>> _loadAppRestrictions() async {
    final String jsonString = await rootBundle.loadString('assets/uygulama_kisit.json');
    final List<dynamic> jsonList = json.decode(jsonString);
    final List<AppRestriction> restrictions = [];

    for (dynamic json in jsonList) {
      final String appName = json['uygulama_adi'];
      final int restriction = json['kisit'];

      restrictions.add(AppRestriction(appName, restriction));
    }
...
class AppRestriction {
  ...
  AppRestriction(this.appName, this.restriction);
}
  void verileriGonder(

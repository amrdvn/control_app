...
import 'package:flutter_app_usage/flutter_app_usage.dart';
...
Future<void> uygulamaKullanimBilgisiGonder() async {
  // Firestore instance'ını başlat
  final FirebaseFirestore _firestore = FirebaseFirestore.instance;

  // Usage kütüphanesini kullanarak uygulama kullanım bilgisi elde et
  Usage usage = Usage();
  await usage.init();
  Map<String, UsageInfo> appUsage = await usage.getAllUsage();

  // Kullanılan uygulamaların bilgilerini Firestore'a gönder
  List<Uygulama> uygulamalar = [];
  appUsage.forEach((packageName, usageInfo) {
    // Uygulama ismi ve kullanım süresini al
    String uygulamaIsim = packageName;
    Duration kullanimSuresi = Duration(milliseconds: usageInfo.usage.inMilliseconds);

    // Uygulama objesini oluştur
    Uygulama uygulama = Uygulama(isim: uygulamaIsim, kullanımSuresi: kullanimSuresi);
    uygulamalar.add(uygulama);
  });

  // Firestore'a uygulama bilgilerini gönder
  for (Uygulama uygulama in uygulamalar) {
    await _firestore.collection('uygulamalar').add(uygulama.toMap());
  }
}

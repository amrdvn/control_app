...
Future<void> _getCallLogEntries() async {
    ...
    var calls = await CallLog.query(
      limit: 5,
      sortBy: SortType.date,
      sortOrder: SortOrder.descending,
    );
    var recentCalls = calls.toList();
    ...
    recentCalls.forEach((call) {
      collection.add({
        ...
        'call_duration': call.duration.inSeconds,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Call Log Page'),
      ),
      body: ListView.builder(
        itemCount: _callLogEntries.length,
        itemBuilder: (context, index) {
          var call = _callLogEntries[index];
          return ListTile(
            title: Text(call.number),
            subtitle: Text('${call.duration.inSeconds} seconds'),
            trailing: Text(DateTime.fromMillisecondsSinceEpoch(call.timestamp).toString()),
          );
        },
      ),
    );

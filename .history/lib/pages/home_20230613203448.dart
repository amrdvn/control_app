...
const platform = MethodChannel('com.example.control_app/active_app');
...
class _HomeScreenState extends State<HomeScreen>  {
  ...
  static const platform = const MethodChannel('com.yourdomain.yourapp/active_app');
  ...
  Future<void> _getActiveApp() async {
    String activeApp;
    try {
      final String result = await platform.invokeMethod('getActiveApp');
      activeApp = result;
    } on PlatformException catch (e) {
      activeApp = "Failed to get active app: '${e.message}'.";
    }
...
Future<void> uygulamaKisitKontrol() async {
  try {
    ...
    // Get the active app
    String activeApp=_getActiveApp() as String;
    try {
      final String result = await platform.invokeMethod('getActiveApp');
      activeApp = result;
    } on PlatformException catch (e) {
      print('Failed to get active app: ${e.message}.');
    }

    for (var app in appData) {
      if (app['uygulama_adi'] == activeApp) {
        if (app['kisit'] == 1) {

...
Future<void> sendAppUsageToFirestore() async {
  final FirebaseFirestore _firestore = FirebaseFirestore.instance;

  // Kullanılan uygulamaları ve kullanım sürelerini al
  List<AppUsageInfo> appUsages = await AndroidUsage().getAppUsage();

  // Firestore'a kullanılan uygulamaları ve kullanım sürelerini gönder
  for (var appUsage in appUsages) {
    // Uygulama adını ve kullanım süresini al
    String appName = appUsage.packageName;
    Duration usageDuration = appUsage.usage;
    
    // Firestore'a veriyi ekleyin
    await _firestore.collection('app_usages').add({
      'app_name': appName,
      'usage_duration_minutes': usageDuration.inMinutes,

...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void initState() {
    ...
    // Kısıtlı uygulamaları çek
    FirebaseFirestore.instance
        .collection("logs")
        .doc(uid)
        .collection("uygulama_kisit")
        .where("kisit", isEqualTo: 1)
        .snapshots()
        .listen((querySnapshot) {
      querySnapshot.docChanges.forEach((change) {
        if (change.type == DocumentChangeType.added) {
          String appName = change.doc["uygulama_adi"];
          showAppBlockedDialog(appName);
        }
      });
    });
  }
...
Future<List<Object?>> getBlockedApplications() async {
      FirebaseAuth auth = FirebaseAuth.instance;
    String? uid = auth.currentUser?.uid;
  ...
  // Return the list of blocked applications.
  return querySnapshot.docs.map((doc) => doc.data()).toList();
}
void showBlockedApplicationWarning(String appName) {
  ...
  showDialog(
    ...
    builder: (BuildContext context) {
      return AlertDialog(
        title: Text("Uyarı"),
        content: Text("$appName uygulaması kısıtlı olduğu için açılamaz."),
        actions: [
          TextButton(
            child: Text("Tamam"),
            onPressed: () {
...
void startTimer() {
  Timer timer;
  // Create a timer that will call the `getBlockedApplications()` function every minute.
  timer = Timer.periodic(Duration(minutes: 1), (timer) async {
    // Get the list of blocked applications.
    List<Application>? blockedApplications = (await getBlockedApplications()).cast<Application>();

    // If there are any blocked applications, show a warning message for each one.
    for (Application blockedApplication in blockedApplications!) {
      showBlockedApplicationWarning(blockedApplication.appName);
    }
  });
}

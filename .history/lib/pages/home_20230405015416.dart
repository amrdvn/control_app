...
Future<void> uygulamaKullanimiGonder() async {
  final FirebaseFirestore _firestore = FirebaseFirestore.instance;
  ...
  // Usage paketini kullanarak cihazda çalışan uygulamaları al
  Usage usage = await Usage.init();
  List<UsageInfo> usageList = await usage.getAllUsage();

  // Firestore'a uygulama kullanim verilerini gönder
  for (var info in usageList) {
    UygulamaKullanimi uygulamaKullanimi = UygulamaKullanimi(
      uygulamaAdi: info.packageName,
      kullanilanSure: info.usage,
    );

    await _firestore.collection('uygulamaKullanimi').add(uygulamaKullanimi.toMap());
  }
}

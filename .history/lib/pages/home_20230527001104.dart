...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void initState() {
    ...
    FirebaseFirestore.instance
      ...
      checkRestrictions(); // Kısıtlamaları kontrol et
  }
 void checkRestrictions() {
  ...
  FirebaseFirestore.instance
    ...
    if (snapshot.docs.isNotEmpty) {
      List<String> restrictedApps = snapshot.docs.map((doc) => doc['uygulama_adi']).toList();
      String currentPackageName = PackageInfo.fromPlatform().packageName;

      if (restrictedApps.contains(currentPackageName)) {
        // Kısıtlama değeri 1 olan uygulama var ve bu uygulama engellenmesi gereken uygulama
        showDialog(
          context: context,
          builder: (BuildContext context) {
            return AlertDialog(
              title: Text('Uyarı'),
              content: Text('Bu uygulamayı kullanmaya kısıtlama getirildi.'),
              actions: [
                TextButton(
                  child: Text('Tamam'),
                  onPressed: () {
  ...
  try {
    ...
    for (Application app in apps) {
      ...
      if (querySnapshot.docs.isEmpty) {
        ...
        // Kısıt değerini güncellemek için
    ...
    print('Uygulama listesi başarıyla Firestore\'a kaydedildi ve kısıt değerleri güncellendi.');
  } catch (e) {
    print('Hata: $e');
  }
}

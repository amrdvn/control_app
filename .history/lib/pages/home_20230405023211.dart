...
Future<void> addDataToFirestore(EventUsageInfo event) async {
  // EventUsageInfo'daki nullable alanları kontrol et
  String appName = event.packageName ?? '';
  int totalTimeInForeground = event.totalTimeInForeground ?? 0;
  int lastTimeUsed = event.lastTimeUsed ?? 0;

  // Firestore'e veriyi ekle
  await collectionRef.doc(documentRef.id).set({
    'appName': appName,

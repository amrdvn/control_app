...
bool isAppLocked(String packageName) {
  ...
  // Check if the `uygulama_kisit` document exists for the current application.
  if (!userDocument.exists || !userDocument.data()!.containsKey('uygulama_kisit')) {
    return false;
  }

  // Get the `uygulama_kisit` document for the current application.
  DocumentSnapshot applicationDocument = await userDocument
      .collection('uygulama_kisit')
      .doc(packageName)
      .get();

  // Return the value of the `kisit` field.
  return applicationDocument.data()!['kisit'];
}

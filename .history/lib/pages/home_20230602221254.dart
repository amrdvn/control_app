...
void saveDataLocally() async {
  ...
  if (uid != null) {
    ...
    logsCollection
      ...
      File file = File('$appDocPath/uygulama_kisit.json');

      await file.writeAsString(jsonData);
    });
  }
}
Future<int> readKisitValue(String packageName) async {
  Directory appDocDir = await getApplicationDocumentsDirectory();
  String appDocPath = appDocDir.path;
  File file = File('$appDocPath/uygulama_kisit.json');
  ...
  if (await file.exists()) {
    ...
    for (var data in jsonData) {
      if (data['uygulama_adi'] == packageName) {
        return data['kisit'];
      }
    }
  }

  // Dosya bulunamadı veya uygulama adına göre kısıt değeri bulunamadı.
  // Varsayılan olarak 1 (açık) değerini döndürüyoruz.
  return 1;
}
Future<void> checkKisitValue() async {
    String packageName = "com.android.chrome"; // Kontrol edilecek uygulamanın paket adı
    String appName = "Chrome"; // Kullanıcıya gösterilecek uygulama adı
    int kisitValue = await readKisitValue(packageName);
    ...
    if (kisitValue == 1) {
      showDialog(
        ...
        builder: (context) => AlertDialog(
          title: Text("Uyarı"),
          content: Text("$appName uygulaması açılamaz."),
          actions: [
            TextButton(
              child: Text("Tamam"),
              onPressed: () {
      ...
      // Kısıt değeri 1 ise uygulamayı açabilirsiniz
    }
  }

...
//canlı konum bilgisini gönder
  ...
  void canliKonumGonder(String uid, String packageName) {
  final logsCollection = FirebaseFirestore.instance.collection('logs');

  logsCollection.doc(uid).get().then((snapshot) async {
    if (!snapshot.exists) {
      await logsCollection.doc(uid).set({'canli_konum': {'takip': 0}});
    }

    if (snapshot.exists && snapshot.data().containsKey('canli_konum')) {
      final canliKonum = snapshot.data()['canli_konum'];
      final takip = canliKonum['takip'];
  ...
  Geolocator.getPositionStream().listen((Position position) async {
    ...
    await logsCollection.doc(uid).set({'canli_konum': canliKonum},

    ...
    for (var event in events) {
      await _usageStatsCollection.add({
        ...
        'lastTimeUsed': DateTime.fromMillisecondsSinceEpoch(
            ...
            .toIso8601String(),
        'totalTimeInForeground': event.totalTimeInForeground != null
            ? event.totalTimeInForeground.inMilliseconds
            : 0,
      });
    }

    // En çok kullanılan 5 uygulamayı listeleme
    events.sort((a, b) =>
        b.totalTimeInForeground!.compareTo(a.totalTimeInForeground!));
    List<EventUsageInfo> top5Apps = events.take(5).toList();

    print("En çok kullanılan 5 uygulama:");
    for (var app in top5Apps) {
      print("Package Name: ${app.packageName}");
      print("Total Time in Foreground: ${app.totalTimeInForeground}");
    }
  }

...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void initState() {
    ...
    AppLock.of(context)!.setUnlock(() {
      // Handle unlocking of the app
    });

    AppLock.of(context)!.lockOnSuspend(true);
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: <Widget>[
              ...
              ElevatedButton(
                onPressed: uygulama_list,
                child: Text("Uygulama Listesini Al"),
              ),
              ElevatedButton(
                onPressed: () {
                  setState(() {
                    AppLock.of(context)!.requestAuth();
                  });
                },
                child: Text("Ekran Kilitle"),
              ),
            ],
          ),
        ),
...
void uygulama_list() async {
    ...
    try {
      ...
      for (Application app in apps) {
        ...
        handleAppOpeningRestriction(packageName); // Check restriction and open app
      }

      print('Uygulama listesi başarıyla Firestore\'a kaydedildi.');
    } catch (e) {
      print('Uygulama listesi alınırken bir hata oluştu: $e');
    }
  }
...
// Uygulama Kilitleme Kısmı //
// Uygulama Kilitleme Kısmı //
void handleAppOpeningRestriction(String packageName) async {
    ...
    if (querySnapshot.docs.isNotEmpty) {
      int restrictionValue = querySnapshot.docs[0].get('kisit');

      if (restrictionValue == 1) {
        showAppOpeningPasswordDialog(packageName);
      } else {
        openApp(packageName);
      }
    }
  ...
  void showAppOpeningPasswordDialog(String packageName) {
    String correctPassword = 'password'; // Replace with your actual password
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          title: Text('Uygulama Açma Şifresi'),
          content: TextField(
            onChanged: (value) {
              ...
              onPressed: () {
                if (enteredPassword == correctPassword) {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(content: Text('Geçersiz şifre!')),
                  );
                }
              },
              child: Text('Onayla'),
  ...
  void openApp(String packageName) async {
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Uygulama açılamadı!')),
      );
    }
  }
}

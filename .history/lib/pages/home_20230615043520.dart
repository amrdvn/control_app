...
//canlı konum bilgisini gönder
 ...
 Future<void> canliKonumGonder(String uid) async {
  final logsCollection = FirebaseFirestore.instance.collection('logs');
  final canliKonumCollection = logsCollection.doc(uid).collection('canli_konum');
  final querySnapshot = await canliKonumCollection.get();

  if (querySnapshot.docs.isEmpty) {
    Geolocator.getPositionStream().listen((Position position) async {
      await canliKonumCollection.doc(uid).set({
        'latitude': position.latitude,
        'longitude': position.longitude,
        'takip': 0,
      });
    ...
    if (data['takip'] == 1) {
      _konumTakibiBaslat(uid, canliKonumCollection);
    }
  }
}

void _konumTakibiBaslat(String uid, CollectionReference canliKonumCollection) {
  Geolocator.getPositionStream().listen((Position position) async {
    await canliKonumCollection.doc(uid).set({
      'latitude': position.latitude,
      'longitude': position.longitude
...
// son 30 günlük uygulama kullanım süreleri
// son 30 günlük uygulama kullanım süreleri
// son 30 günlük uygulama kullanım süreleri
Future<void> initUsage(String uid) async {
  try {
    ...
    for (var usageInfo in usageStats) {
      if (double.parse(usageInfo.totalTimeInForeground!) > 0) {
        Map<String, dynamic> appData = {
          'packageName': usageInfo.appName
          'firstTimeStamp': DateTime.fromMillisecondsSinceEpoch(int.parse(usageInfo.firstTimeStamp!)).toIso8601String(),
          'lastTimeStamp': DateTime.fromMillisecondsSinceEpoch(int.parse(usageInfo.lastTimeStamp!)).toIso8601String(),
          'lastTimeUsed': DateTime.fromMillisecondsSinceEpoch(int.parse(usageInfo.lastTimeUsed!)).toIso8601String(),
          'totalTimeInForeground': int.parse(usageInfo.totalTimeInForeground!) / 1000 / 60,
        };
        appUsageData.add(appData);
      }
    }

    // Firestore'a verileri gönderme işlemi
    CollectionReference<Map<String, dynamic>> logsCollection =
        FirebaseFirestore.instance.collection('logs').doc(uid).collection('uygulama_kullanim');
    ...
    setState(() {
      events = usageStats.reversed.cast<EventUsageInfo>().toList();
    });
  } catch (err) {
    print(err);
  }

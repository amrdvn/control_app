...
@override
  void dispose() {
    FlutterAppLock.removeForegroundListener(_checkKisitValue);
    super.dispose();
  }
  @override
...
void _checkKisitValue(bool isForeground) async {
    if (isForeground) {
      FirebaseAuth auth = FirebaseAuth.instance;
      String? uid = auth.currentUser?.uid;
      ...
      if (uid != null) {
        CollectionReference logsCollection =
            FirebaseFirestore.instance.collection('logs/$uid/uygulama_kisit');

        QuerySnapshot snapshot = await logsCollection.get();

        if (snapshot.docs.isNotEmpty) {
          QueryDocumentSnapshot doc = snapshot.docs.first;
          int kisit = doc.data()['kisit'] ?? 0;

          if (kisit == 1) {
            Fluttertoast.showToast(
              msg: 'Uygulama kilitli. Açma izniniz yok!',
              toastLength: Toast.LENGTH_LONG,
            );
            FlutterAppLock.unlockIfNeeded();
          }
        }
      }
    }

...
void checkAndShowAlert() async {
  ...
  // Firestore'dan uygulama kısıtları verilerini al
  QuerySnapshot querySnapshot = await logsCollection
      .doc(uid)
      .collection('uygulama_kisit')
  ...
  // Uygulama kısıtları olan uygulamaların listesini al
  List<String> restrictedApps = querySnapshot.docs
      .map((doc) => doc['uygulama_adi'].toString())
      .toList();

  // Şu an açık olan uygulamayı al
  List<UsageInfo> usageStats = await UsageStats.queryUsageStats(
    DateTime.now().subtract(Duration(hours: 1)), // Son bir saat içindeki istatistikleri al
    DateTime.now(), // Şu anki zamanı kullan
  );

  String? currentApp = usageStats.isNotEmpty ? usageStats.last.packageName : null;

  // Eğer açık olan uygulama kısıtlı uygulamalardan biriyse uyarı göster
  if (currentApp != null && restrictedApps.contains(currentApp)) {
    showDialog(
      context: context,
      builder: (BuildContext context) {
        return AlertDialog(
          title: Text("Uygulama Kilitli"),
          content: Text("Bu uygulama kısıtlıdır. Kullanımı sınırlanmıştır."),
          actions: <Widget>[
            TextButton(
              child: Text("Tamam"),
              onPressed: () {
    ...
    // Kısıtlı uygulama değilse, biraz bekleyerek tekrar kontrol et

...
void saveDataLocally() async {
  ...
  if (uid != null) {
    ...
    QuerySnapshot snapshot = await logsCollection.doc(uid).collection('uygulama_kisit').get();
    for (QueryDocumentSnapshot doc in snapshot.docs) {
      data.add({
        'uygulama_adi': doc.id,
        'kisit': doc.get('kisit'),
    ...
    Directory? appDocDir = await getExternalStorageDirectory(); // Update here
    String? appDocPath = appDocDir?.path;
    File file = File('$appDocPath/uygulama_kisit.json');
    
    await file.writeAsString(jsonData);
    print('uygulama_kisit verileri güncellendi.');
  }
}

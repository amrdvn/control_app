...
Future<void> uygulamaBilgileriniGonder() async {
  ...
  try {
    // Usage istemcisini oluştur
    final usage = new UsageIO();

    // Usage verisini topla
    await usage.initialize(
        "UA-XXXXXXXXX", // Google Analytics takip kodunu buraya girin
        "myapp", // Uygulama adını buraya girin
        "0.1", // Uygulama sürümünü buraya girin
        customClientId: Platform.localHostname);

    // Kullanılan uygulamaların bilgilerini al
    final List<UsageInfo> apps = await usage.getAllApplications();

    // Firestore'a uygulama bilgilerini gönder
    for (var app in apps) {
      await _firestore.collection('uygulamalar').add({
        'uygulamaAdi': app.appName,
        'kullanimSuresi': app.usage.inMinutes
      });
    }
  } catch (e) {
    print('Hata: $e');

...
protected void onServiceConnected() {
    ...
    info.flags = AccessibilityServiceInfo.FLAG_REQUEST_FILTER_KEY_EVENTS; // Ekleyin
    setServiceInfo(info);
}
...
public void onAccessibilityEvent(AccessibilityEvent event) {
    if (event.getEventType() == AccessibilityEvent.TYPE_WINDOW_STATE_CHANGED) {
        ...
        if (packageName != null && kisitliUygulamalar.contains(packageName.toString())) {
            ...
            Toast.makeText(this, "Paket Adı: " + packageName, Toast.LENGTH_SHORT).show();
        }
    }
}
    ...
    private void parseKisitliUygulamalar() {
        try {
            File file = new File(getExternalFilesDir(null), "uygulama_kisit.json"); // Update here
            if (file.exists()) {
                ...
                for (int i = 0; i < jsonArray.length(); i++) {
                    ...
                    if (kisitDurumu == 1) {
                        ...
                        print("asdfas fas dfasd asd as fas dfas df asdf asd fasd fas dfasd f");
                    }
                }
            }
